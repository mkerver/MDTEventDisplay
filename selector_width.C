...
*/
   ...
   EveYX = new TH2F("EveYX", "EveYvX", 10, -11.0, 11.0, 100, 6.5, 7.7); 
   rpcHits_YX = new TH2F("rpcHits_YX", "hitsYvX", 1000, -11.0,11.0, 100, 6.5, 7.7); 
   mdtHits_YX = new TH2F("rpcHits_YX", "hitsYvX", 1000, -11.0,11.0, 100, 6.5, 7.7); 
   trkHits_YX = new TH2F("trkHits_YX", "hitsYvX", 1000, -11.0,11.0, 100, 6.5, 7.7); 
...
if(nevents!=1400) return kTRUE;
  ...
  float mmToM = 1./1000.;
  int nMdtHits = 0;
//  cout<<"event: "<<event_eventNumber<<" run: "<<event_runNumber<<endl; 
     ...
     for(int k=0;k<trkHit_nHits;k++){
       ...
       if(trackindex==i){
         if(chambertype[0]=='B' && chambertype[1]=='M' && chambertype[2]=='L' && sector==3 && eta<6){
           ...
           trkHits_YX->Fill(trkHit_posX[k]*mmToM,trkHit_posY[k]*mmToM);
           if(TechID(mfid)==3){
             rpcHits_YZ->Fill(trkHit_posZ[k]*mmToM,trkHit_posY[k]*mmToM);
             rpcHits_YX->Fill(trkHit_posX[k]*mmToM,trkHit_posY[k]*mmToM);
...
//loop over rawmdt hits
  ...
  for(int m=0; m<rawMdt_nRMdt;m++){
     ...
     if(rawMdt_adc[m]>50){
        ...
        mdtHits_YX->Fill(rawMdt_gPosX[m]*mmToM, rawMdt_gPosY[m]*mmToM);
      }
    }
...
void selector::Terminate() {
  ...
  TCanvas *c5 = new TCanvas("c5","EveYvX",700,900);
  ...
  EveYX->SetMarkerColor(21);
  rpcHits_YX->SetMarkerColor(kRed);
  mdtHits_YX->SetMarkerColor(kBlue);
  trkHits_YX->SetMarkerColor(kGreen+2);
  mdtHits_YX->SetMarkerSize(0.5);
  rpcHits_YX->SetMarkerSize(.5);
  trkHits_YX->SetMarkerSize(.8);
  trkHits_YX->SetMarkerStyle(5);
  mdtHits_YX->SetMarkerStyle(8);
  rpcHits_YX->SetMarkerStyle(8);
  mdtHits_YX->Draw("same");
  trkHits_YX->Draw("same");
  rpcHits_YX->Draw("same");
